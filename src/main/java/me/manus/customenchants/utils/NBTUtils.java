...
public class NBTUtils {
    ...
    public static void removeEnchant(ItemStack item, String enchantId) {
        ...
        String current = meta.getPersistentDataContainer().getOrDefault(ENCHANT_KEY, PersistentDataType.STRING, "");
        if (current.contains(enchantId)) {
            String updated = java.util.Arrays.stream(current.split(","))
                    .filter(id -> !id.equalsIgnoreCase(enchantId))
        ...
        // Limpar XP e Stats relacionados
        meta.getPersistentDataContainer().remove(new NamespacedKey(pluginInstance, "exp_" + enchantId));
        // Nota: Stats genéricos como 'explosive_blocks' não são removidos pois são do item, não do encantamento específico,
        // mas o XP é zerado para que se o item for re-encantado comece do zero.
